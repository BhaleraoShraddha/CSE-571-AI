...
class PlanRouteProblem(search.Problem):
    ...
    def h(self,node):
        ...
        # d.append(0)
        for i in self.goals:
            d.append(manhattan_distance_with_heading(node.state, i))
        return max(d)
        # return 0
    ...
    def actions(self, state):
        ...
        y = state[1]
        heading = state[2]
        fwd = ['Forward', 'Grab', 'Shoot', 'Climb','TurnLeft', 'TurnRight', 'Wait']
        nofwd = ['Grab', 'Shoot', 'Climb', 'TurnLeft', 'TurnRight', 'Wait']
        if (heading == 0 and (x, y + 1) in self.allowed) or (heading == 1 and (x - 1, y) in self.allowed)\
            or (heading == 2 and (x, y - 1) in self.allowed) or (heading == 3 and (x + 1, y) in self.allowed):
            return fwd
        return nofwd
    ...
    def result(self, state, action):
        ...
        heading = state[2]
        # print action
        possibleActions = ['Forward','TurnLeft', 'TurnRight']
        if(action == 'Wait' or action == 'Grab' or action == 'Shoot' or action == 'Climb'):
            return state
        elif action == 'Forward':
            if heading == 0:
